...
class ThreeSample {
  constructor() {
    ...
    //自作クラスのイベントを実体化
    this.matterEvents = new MatterEvents();
  }
...
class MatterSphereMesh {
  constructor(scene, x, y, w) {
    ...
    //matterjsのイベントでthreejsのメシュを操作するために。
    //matterBodyの中に入れる
    this.matterBody.ball = new THREE.Mesh(this.geometry, this.material);
    this.matterBody.ball.position.x = x - canvasWidth / 2;
    this.matterBody.ball.position.y = -(y - canvasHeight / 2);
    scene.add(this.matterBody.ball);
    World.add(world, this.matterBody);
  }
  tick() {
    let pos = this.matterBody.position;
    let angle = this.matterBody.angle;
    this.matterBody.ball.position.x = pos.x - canvasWidth / 2;
    this.matterBody.ball.position.y = -(pos.y - canvasHeight / 2);
    this.matterBody.ball.rotation.z = -angle;
    //下に落ちたら上にワープさせる
    if (pos.y > 350) {
